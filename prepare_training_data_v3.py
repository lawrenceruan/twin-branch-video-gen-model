...
def main():
    ...
    # ── check for existing manifest and resume ──────────────────────────
    ...
    if os.path.exists(manifest_path):
        with open(manifest_path) as f:
            for line in f:
                if line.strip():
                    rec = json.loads(line)
                    # Extract video path from the original sample to track processed items
                    if "video_path" in rec:
                        processed_videos.add(rec["video_path"])
        logging.info(f"Found existing manifest with {len(processed_videos)} processed samples. Resuming...")

    for idx, sample in enumerate(samples):
        video_path = sample["video_path"]
        text_prompt = sample["text_prompt"]
        formatted_text_prompt = model_specs["formatter"](text_prompt)
        image_path = sample.get("image_path", None)

        # Skip if already processed
        if video_path in processed_videos:
            logging.info(f"[{idx+1}/{len(samples)}] SKIP (already processed): {video_path}")
            continue
        ...
        # ── text ─────────────────────────────────────────────────────────
        with torch.no_grad():
            text_emb = text_model([formatted_text_prompt], text_model.device)[0]  # [L, D]
        ...
        waveform = waveform.to(device=device, dtype=torch.bfloat16)  # Keep as [1, T]
        with torch.no_grad():
        ...
        # Use video filename (without extension) as the pt filename
        video_basename = os.path.splitext(os.path.basename(video_path))[0]
        pt_path = os.path.join(latent_dir, f"{video_basename}.pt")
        ...
        # ── append to manifest immediately after processing each sample ──
        with open(manifest_path, "a") as f:
            f.write(json.dumps(record) + "\n")
        
        logging.info(f"  → Saved {pt_path} and updated manifest")

    logging.info(f"Done. All samples processed → {manifest_path}")
